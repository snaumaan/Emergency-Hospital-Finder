...
async function loadAppointments(filterStatus) {
  // Use apptTableFull for the full appointments page, apptTableOverview for overview
  const isOverview = !filterStatus && document.getElementById('page-overview').classList.contains('active');
  const elId = isOverview ? 'apptTableOverview' : 'apptTableFull';
  const el = document.getElementById(elId);
  if (!el) return;

  el.innerHTML = '<div class="spinner">Loading appointments…</div>';
  try {
    const url = '/api/appointments/hospital' + (filterStatus && filterStatus !== 'all' ? '?status=' + filterStatus : '');
    const d = await api('GET', url);
    ALL_APPOINTMENTS = d.appointments || [];
    renderAppointmentTable(ALL_APPOINTMENTS, elId);
  } catch (e) {
...
function filterAppointments(status) {
  ...
  const filtered = status === 'all'
    ? ALL_APPOINTMENTS
    : ALL_APPOINTMENTS.filter(a => a.status === status);
  renderAppointmentTable(filtered, 'apptTableFull');
}

function renderAppointmentTable(appts, elId) {
  const el = document.getElementById(elId);
  if (!el) return;
  if (appts.length === 0) {
    el.innerHTML = '<div class="empty">No appointments found.</div>';
    return;
  }
  el.innerHTML = `<table style="width:100%;border-collapse:collapse;">
    <thead><tr style="border-bottom:1px solid var(--border);">
      <th style="text-align:left;padding:10px 12px;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:rgba(250,247,242,0.35);">Token</th>
      <th style="text-align:left;padding:10px 12px;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:rgba(250,247,242,0.35);">Patient</th>
      <th style="text-align:left;padding:10px 12px;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:rgba(250,247,242,0.35);">Department</th>
      <th style="text-align:left;padding:10px 12px;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:rgba(250,247,242,0.35);">Date</th>
      <th style="text-align:left;padding:10px 12px;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:rgba(250,247,242,0.35);">Status</th>
      <th style="text-align:left;padding:10px 12px;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:rgba(250,247,242,0.35);">Actions</th>
    </tr></thead>
    <tbody>${appts.map(a => appointmentRow(a)).join('')}</tbody>
  </table>`;
}
...
function appointmentRow(a) {
  ...
  return `<tr style="border-bottom:1px solid rgba(255,255,255,0.04);"
    ...
    <td style="padding:12px;">
      <div style="font-size:13px;color:var(--cream);">${name}</div>
