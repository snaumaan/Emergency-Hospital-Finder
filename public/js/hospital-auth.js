...
async function handleHospLogin() {
  ...
  try {
    ...
    showError('hLoginError', e.message);
  } finally {
    setBusy('hLoginBtn', false, 'Sign In →');
  }
}
...
async function handleHospRegister() {
  ...
  try {
    ...
    showError('hRegError', e.message);
  } finally {
    setBusy('hRegBtn', false, 'Register Hospital →');
  }
}
...
// ─── BOOT: runs after all scripts are loaded via DOMContentLoaded ─────────────
