...
function renderBeds(beds) {
  ...
  el.innerHTML = beds.map(b => {
    const i   = info[b.type] || { label: b.type, icon: '🛏️', color: 'var(--gray)' };
    ...
    return `<div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:24px;">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
        ...
        <div>
          <div style="font-family:Syne,sans-serif;font-size:15px;font-weight:600;color:var(--white);">${i.label}</div>
      ...
      <div style="background:rgba(255,255,255,0.06);border-radius:4px;height:6px;margin-bottom:16px;overflow:hidden;">
        <div style="width:${pct}%;height:100%;background:${i.color};border-radius:4px;transition:width .4s;"></div>
      </div>
      <div style="display:flex;align-items:center;justify-content:space-between;">
        <span style="font-family:Syne,sans-serif;font-size:32px;font-weight:800;color:${i.color};">${b.available}</span>
...
function renderDepartments() {
  ...
  if (depts.length === 0) { el.innerHTML = '<div class="empty">No departments yet.</div>'; return; }
...
function renderAnalytics() {
  ...
  const depts = HOSPITAL.departments || [];
  const total = beds.reduce((s, b) => s + b.total, 0);
  const avail = beds.reduce((s, b) => s + b.available, 0);
  const occ   = total > 0 ? Math.round(((total - avail) / total) * 100) : 0;
  el.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px;">
      <div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:20px;">
        <div style="font-size:12px;color:rgba(250,247,242,0.35);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">Bed Occupancy</div>
        <div style="font-family:Syne,sans-serif;font-size:42px;font-weight:800;color:var(--white);">${occ}<span style="font-size:20px;">%</span></div>
        <div style="background:rgba(255,255,255,0.06);border-radius:4px;height:6px;margin-top:12px;overflow:hidden;">
          <div style="width:${occ}%;height:100%;background:${occ>80?'var(--red)':occ>60?'var(--amber)':'var(--green)'};border-radius:4px;"></div>
        </div>
      </div>
      <div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:20px;">
        <div style="font-size:12px;color:rgba(250,247,242,0.35);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">Departments</div>
