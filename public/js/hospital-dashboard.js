...
function populateHospUI() {
  ...
  if (el('hospType'))        el('hospType').textContent        = HOSPITAL.hospitalType || 'Hospital';
...
async function loadHospStats() {
  try {
    ...
    if (erEl) {
      erEl.textContent   = s.erStatus ? 'Open' : 'Closed';
      erEl.style.color   = s.erStatus ? 'var(--green)' : 'var(--gray)';
    }
  } catch (e) {
...
async function toggleER() {
  ...
  try {
    ...
    toggle.checked = !toggle.checked; // revert on fail
